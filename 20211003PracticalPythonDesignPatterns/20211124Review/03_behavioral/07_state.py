...
class TrafficLightRed(IState):
    def handle1(self) -> None:
        print(f'{self.__class__}: handle request1, i will change the state:')
        self.context.transition_to(TrafficLightGreen())

    def handle2(self) -> None:
        print(f'{self.__class__}: handle the request 2;')
...
class TrafficLightGreen(IState):
    ...
    def handle2(self) -> None:
        print(f'{self.__class__}: handle the request1, and change the state:')
        self.context.transition_to(TrafficLightGreen())