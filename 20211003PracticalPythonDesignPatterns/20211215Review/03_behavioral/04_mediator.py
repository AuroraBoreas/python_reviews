...
class IMediator(ABC):
    @abstractmethod
    def notifiy(self, sender:Component, event:str)->None:
        pass
...
class Mediator(IMediator):
    ...
    def notifiy(self, sender: Component, event: str) -> None:
        ...
        if event == 'A':
            print(f'{self.__class__} : received the info and will do the following:')
            self._comp1.do_c()

        elif event == 'D':
            print(f'{self.__class__} : receivedd the info and will do the following..')
            self._comp1.do_a()
            self._comp1.do_b()
